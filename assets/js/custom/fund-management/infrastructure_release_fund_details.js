...
fetchReleasedFundDetails = (id) => {
    var url = conf.getInfrastructureReleasedFundDetails.url(id);
    var method = conf.getInfrastructureReleasedFundDetails.method;
    var data = {};
...
fillFields = (data) => {
    ...
    if(released_details.length){
        ...
        released_details.forEach(function(row){
            ...
            html +='<td class="text-right">'+ utils.formatCurrency(user_wise_balance[row.created_by_id]) +'</td>';
            html +='</tr>';
        });
    }
    else{
        html +='<tr><td colspan="10">No Release Fund Details</td></tr>';
    }
