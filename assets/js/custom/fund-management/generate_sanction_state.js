...
$(document).ready(function() {
    ...
    $('#sanctioned_amount').on('keyup',function(){
        ...
        $('#remaining_balance_amount').html(utils.formatCurrency(remaining_balance_amount));
    })
});
fetchSanctionDetails = (id) => {
...
fillFields = (data) => {
    ...
    $('#maximum_sanction_percent').html(utils.formatCurrency(maximum_sanction_percent));
    var maximum_can_sanctioned=(approved_amount*maximum_sanction_percent)/100;
    maximum_can_sanctioned=maximum_can_sanctioned.toFixed(4);
    var balance_can_sanctioned=maximum_can_sanctioned-data.sanctioned_amount;
    balance_can_sanctioned=balance_can_sanctioned.toFixed(4);
    $('#maximum_can_sanctioned').html(utils.formatCurrency(maximum_can_sanctioned));
...
$("#formID").submit(function(e) {
        ...
        submitHandler: function(form) { 
            ...
            var url = conf.addSanctionLetterState.url;
            var method = conf.addSanctionLetterState.method;
            if (id != undefined && id != '') 
            {
                data.append('id', id );
            }
...
{
    var url = conf.viewMfpProcurementSanctionHistory.url(id);
    var method = conf.viewMfpProcurementSanctionHistory.method;
    var data = {};
    TRIFED.asyncAjaxHit(url, 'GET', data, function (response, cb) {
        if (response) {
            ...
            response.data.forEach(function(row){
                ...
                html +='<td>'+ row.sanctioned_amount +'</td>';
