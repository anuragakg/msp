 ...
 * 2. Mail
 */
class MfpProcurementFundReleased extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {
        $reference_number=$this->reference_number;
        $this->action .= "fund-management/mfp_procurement_release_fund.php";
        
        return [
            'message' => 'MFP Procurement fund released for consolidated id '.$reference_number.' of Rs. '.$this->release_amount.' done by '.$this->from->name,
            'action' => $this->action
        ];
    }
    ...
    public function toSms($notifiable)
    {
        ...
        return sprintf(
            'Dear Sir/Madam, MFP Procurement fund released for consolidated id  '.$reference_number.' of Rs. '.$this->release_amount.' done by '.$this->from->name.'.'
        );
    }
}
