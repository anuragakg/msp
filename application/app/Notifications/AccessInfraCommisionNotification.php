 ...
 * 2. Mail
 */
class AccessInfraCommisionNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject('Infrastructure commission amount is exceded from max aggregate for consolidated id '.$this->reference_number )
            ->line('Dear Sir/Madam,')
            ->line('Greetings!!,')
            ->line('Infrastructure commission amount is exceded from max aggregate .Please find below details')
    ...
    public function toArray($notifiable)
    {
        ...
        return [
            'message' => 'Infrastructure commission amount is exceded from max aggregate for consolidated id '.$this->reference_number.' .',
            'action' => $this->action
        ];
    }
    ...
    {
        return sprintf(
            'Infrastructure commission amount is exceded from max aggregate for consolidated id '.$this->reference_number.' .'
        );
    }
}
