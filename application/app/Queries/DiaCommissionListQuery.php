...
use App\Models\Mfp_procurement_dia_release_bank as ServiceModel;
use Illuminate\Database\Eloquent\Builder;
use DB;
class DiaCommissionListQuery extends BaseQuery
{
    ...
    private function getAdminData($user,$request)
    {
        ...
        return ServiceModel::where($where)->where('commission_amount','>',0)->whereHas('getMfpProcurement', function (Builder $query) use ($user,$request) {
    ...
    private function getDiaData($user,$request)
    {
        ...
        return ServiceModel::where($where)->where('commission_amount','>',0)->where('created_by', $user->id)->whereHas('getMfpProcurement', function (Builder $query) use ($user,$request) {
                if(isset($request['search']['value']) && !empty($request['search']['value']))
                {
                    $search = $request['search']['value'];     
                    $query->where(DB::raw("CONCAT(`proposal_id`)"), 'LIKE', "%".$search."%");
    ...
    private function getNodalData($user,$request)
    {
        ...
        })->whereHas('getUserDetails', function (Builder $query) use ($user,$request) {
